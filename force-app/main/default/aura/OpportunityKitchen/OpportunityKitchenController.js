({
    ...
    handleOnIsActiveChange: function(component, event, helper) {
        ...
        if (isActive && (currentPhase != 'init')) {
            ...
            if (tabset!=null) {
                tabset.set("v.selectedTabId", "KDetailsTab");
            }
        }
    },
    ...
    handleMainSubmit : function(component, event, helper) {
        ...
        var kitchen = event.getParam('fields');
        kitchen.Id = component.get("v.kitchenId");
        kitchen.KitchenManager__c = component.get("v.kManager");
        kitchen.KitchenDesigner__c = component.get("v.kDesigner");
        kitchen.Opportunity__c = component.get("v.opportunityId");
        if (helper.validateProject(component)) {
            component.set("v.showSpinner", true);
            component.find('kitchenForm').submit(kitchen);
        }
    }, // handleMainSubmit

    handleOnSuccess : function(component, event, helper) {
        // The master record has been saved. Now save the request lines.
        var oldId = component.get("v.kitchenId");
        var record = event.getParam("response");
        var recordId = record.id;         
        component.set("v.kitchenId", recordId);
        helper.kitchenSaved(component);
    }, // handleSave
