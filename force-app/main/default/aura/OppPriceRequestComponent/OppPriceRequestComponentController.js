({
    ...
    changeProductData : function(component, event, helper) {
        component.set("v.showSpinnerItemForm", true);
        var opportunityProducts = component.get("v.opportunityProducts");
        var pricebook = component.get("v.pricebook");
        var tariffPrices = component.get("v.tariffPrices");
        var referencePrices = component.get("v.referencePrices");
        var productItemSelected = component.find('OpportunityProduct__c').get("v.value");
        var opportunityId = component.get("v.recordId");
        var isFactoryManager = component.get("v.isFactoryManager");
        var aux = this;
        var customerPrice = null;
        var noDefaultUnitPrice = false;
        
        if (productItemSelected == '') {
            component.set('v.itemFields[0].itemValue', null);
            component.set('v.itemFields[1].itemValue', null);
            component.set('v.itemFields[2].itemValue', null);
            component.set('v.itemFields[3].itemValue', null);
            component.set('v.itemFields[4].itemValue', null);
            component.set('v.itemFields[5].itemValue', null);
            component.set('v.itemFields[6].itemValue', null);
            component.set('v.itemFields[7].itemValue', null);
            component.set('v.itemFields[8].itemValue', null);
            component.set('v.itemFields[9].itemValue', null);
            component.set('v.itemFields[10].itemValue', null);
            component.set('v.itemFields[11].itemValue', null);
            component.set("v.showSpinnerItemForm", false);
        } else {
            var productSelectedData = opportunityProducts.filter(elem => elem.Id == productItemSelected);
            if (productSelectedData.length > 0) {
                //var standardPrice = tariffPrices[0][productSelectedData[0].ProductCode] != null ? tariffPrices[0][productSelectedData[0].ProductCode].UnitPrice : null;
                if (customerPrice == null && referencePrices.length > 0 && referencePrices[0][productSelectedData[0].ProductCode] != null) {
                    customerPrice = referencePrices[0][productSelectedData[0].ProductCode].UnitPrice
                }
            }

            component.set('v.itemFields[0].itemValue', productSelectedData[0].CodigoAuxiliar__c == null ? productSelectedData[0].ProductCode : productSelectedData[0].CodigoAuxiliar__c );
            component.set('v.itemFields[1].itemValue', productSelectedData[0].DescripcionAuxiliar__c == null ? productSelectedData[0].Product2.Name : productSelectedData[0].DescripcionAuxiliar__c);
            component.set('v.itemFields[2].itemValue', (productSelectedData[0].Quantity).toString());
            if (referencePrices.length > 0 && referencePrices[0][productSelectedData[0].ProductCode] != null) {
                component.set('v.itemFields[3].itemValue', referencePrices[0][productSelectedData[0].ProductCode].UnitPrice);
            } else {
                component.set('v.itemFields[3].itemValue', null);
            }
            console.log(tariffPrices.length);
            console.log(tariffPrices[0]);
            console.log(tariffPrices[0][productSelectedData[0].ProductCode]);
            console.log(tariffPrices[0][productSelectedData[0].ProductCode].Uprice__c);
            if (tariffPrices.length > 0 && tariffPrices[0][productSelectedData[0].ProductCode].Uprice__c != null) {
                component.set('v.itemFields[7].itemValue', tariffPrices[0][productSelectedData[0].ProductCode].Uprice__c);
            } else {
                noDefaultUnitPrice = true;
                component.set('v.itemFields[7].itemValue', 'UN');
            }
            
            component.set('v.itemFields[4].itemValue', customerPrice);
            component.set('v.itemFields[6].itemValue', tariffPrices[0][productSelectedData[0].ProductCode].CurrencyIsoCode);
            component.set('v.itemFields[8].itemValue', tariffPrices[0][productSelectedData[0].ProductCode].Product2Id);
            component.set('v.itemFields[9].itemValue', pricebook != null ? pricebook.Id : null);
            component.set('v.itemFields[12].itemValue', isFactoryManager);

            Promise.all([
                helper.updateOppProductPriceAjax(component, opportunityId,productItemSelected),
                helper.updateOppProductFactoryPriceAjax(component, opportunityId,productSelectedData[0].Id)
            ]).then(function(results) {
                var storePrice = results[0];
                var factoryPrice = results[1];

                console.log(storePrice);
                console.log(factoryPrice);

                if (storePrice != null) {
                    if (storePrice.retailValue != null) {
                        //component.set('v.itemFields[3].itemValue', storePrice.retailValue);
                    }
                    if (storePrice.customerValue != null && pricebook != null && pricebook.IsRetailReferencePricebook__c == true) {
                        console.log('Cambio precio Customer')
                        component.set('v.itemFields[4].itemValue', storePrice.netValue);
                    }
                }

                if (factoryPrice != null) {
                    if (factoryPrice.netValue != null) {
                        component.set('v.itemFields[10].itemValue', factoryPrice.netValue);
                        component.set('v.itemFields[10].disabled', true);
                    }

                    if (noDefaultUnitPrice && factoryPrice.retailIsoUnit != null) {
                        component.set('v.itemFields[7].itemValue', factoryPrice.retailIsoUnit);
                    }
                } else {       
                ...
                component.set("v.showSpinnerItemForm", false);
                $A.enqueueAction(component.get('c.collapsePanel'));
            });
        }
    },
    ...
    saveItem : function(component, event, helper) {
        ...
        if (!approverMode) {
            ...
            console.log('AJDLASJDLASDLSLDJS')
            console.log(auxItem['Price__c'])
            console.log(auxItem['PriceNeeded__c'])
            if (auxItem['Price__c'] > 0 && (auxItem['PriceNeeded__c'] >= auxItem['Price__c'] || auxItem['PriceNeeded__c'] >= auxItem['Price__c'])) {
                formErrorText += $A.get('$Label.c.prequest_price_needed_inferior') + ' ';
                formError = true;
            }

            if (auxItem['Price__c'] > 0 && (auxItem['PriceNeeded__c'] < (auxItem['ReferencePrice__c']*0.30))) {
