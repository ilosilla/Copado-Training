({
    loadAllData:  function(component) {
        console.log('VAMOS A CARGAR TODO');
        ...
        this.getAllDataAjax(component, component.get("v.recordId")).then(function(result){
            ...
            var isFactoryUser = result.permissions.isFactoryManager == true || result.permissions.isFactoryApprover == true;
            ...
            if (result.pricebook.IsStandard == true) {
                component.set("v.isStandardPricebook", true);
            }
            ...
            console.log(result.permissions)
             if (result.permissions.isEnabled == false) {
                ...
                var itemFields = [
                    ...
                    {field: 'PriceNeeded__c', defaultValue: '', disabled: approverMode, required: true, typeField: 'number', hide: false},
                    ...
                    {field: 'PriceBook__c', defaultValue: '', disabled: true, required: true, typeField: 'text', hide: true},
                    {field: 'FactoryPrice__c', defaultValue: '', disabled: true, required: isFactoryUser, typeField: 'number', hide: !isFactoryUser},
                    {field: 'FactoryPriceNeeded__c', defaultValue: '', disabled: !isFactoryUser, required: isFactoryUser, typeField: 'number', hide: !isFactoryUser},
                    {field: 'FactoryRequest__c', defaultValue: false, disabled: true, required: false, typeField: 'boolean', hide: true}
                ];

                if (result.pricebook.IsStandard == true) {
                    itemFields[3].hide = true; // Hide ReferencePrice__c if standard pricebook
                    itemFields[4].hide = true; // Hide Price__c if standard pricebook
                    itemFields[4].required = false; // Hide Price__c if standard pricebook
                    itemFields[5].hide = true; // Hide PriceNeeded__c if standard pricebook
                    itemFields[5].required = false; // Hide PriceNeeded__c if standard pricebook

                    //itemFields[7].disabled = false; // Disable UnitOfPrice__c if standard pricebook
                }
    ...
    submitItemsForApprovalAjax : function(component, items) {
        ...
        if (component.get("v.isFactoryManager")) {
            var action = component.get("c.itemsForFactoryApproval");
        } else {
            console.log('store')
            var action = component.get("c.itemsForStoreApproval");
        }
    ...
    submitApprovedAjax : function(component, approved) {
        var action = component.get("c.itemsApproved");
        action.setParams({
    ...
    updateOppProductFactoryPriceAjax : function(component, opportunityId, productLineId) {
        ...
        var fields = component.find("inputField");

        var unitPriceValue;
        if (Array.isArray(fields)) {
            fields.forEach(function(fieldCmp) {
                if (fieldCmp.get("v.fieldName") === "UnitOfPrice__c") {
                    unitPriceValue = fieldCmp.get("v.value");
                }
            });
        } else {
            if (fields.get("v.fieldName") === "UnitOfPrice__c") {
                unitPriceValue = fields.get("v.value");
            }
        }
        ...
        action.setParams({
            ...
            oppProductLineId: productLineId,
            Unit: unitPriceValue,
            useUnit: unitPriceValue != null && unitPriceValue != ''
        });
        return this.returnPromiseAjax(action);
    },
